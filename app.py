...
from model import train_model_background, extract_embedding_for_image, load_model_if_exists, predict_with_model, MODEL_PATH
...
def write_train_status(status_dict):
    # Lock so concurrent writes from the background thread and the request
    # thread can't interleave and corrupt the JSON file.
...
def train_model_route():
    ...
    def progress_cb(pct, message, done=False):
        # `done` tells us whether training has actually finished (success,
        # failure, or "no data") so we know when to flip `running` back off.
        write_train_status({"running": not done, "progress": pct, "message": message})

    def run_training():
        try:
            train_model_background(DATASET_DIR, progress_cb)
        except Exception as e:
            app.logger.exception("training failed")
            write_train_status({"running": False, "progress": 0, "message": f"Training failed: {e}"})
